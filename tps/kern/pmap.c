...
mem_init(void)
{
	...
	size_t size_envs = sizeof(struct Env) * NENV;
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	int start_page_io = IOPHYSMEM / PGSIZE;
	int end_page_kernel = PADDR(boot_alloc(0)) / PGSIZE;

	// La pagina 0 se reserva para la BIOS, arranco desde la pagina 1
	for (i = 1; i < npages; i++) {
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while (size_count > 0) {
		...
		if (va % PTSIZE == 0 && pa % PTSIZE == 0) {
			pgdir[PDX((uint32_t) va)] =
			        (uint32_t) pa | PTE_P | PTE_PS | perm;
			...
			if ((page_table_entry =
			             pgdir_walk(pgdir, (void *) (va), 1)) == NULL)
				panic("No hay mas memoria.");
...
{
	// LAB 3: Your code here.
	uint32_t virtual_address = (uint32_t) va;
